...
csv_logger = CSVLogger('lstm_training_log.csv', append=False, separator=';')
history = lstm_model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=50, batch_size=32, callbacks=[csv_logger])

with open('lstm_training_results.txt', 'w') as f:
...
sns.heatmap(conf_mat, annot=True, fmt="d", cmap="Blues", xticklabels=['5-Methylcytosine', 'Adenine', 'Cytosine', 'Guanine', 'Thymine1', 'Uracil'],
...
plt.show()
lstm_model.save('lstm_model_weights.h5')
plt.figure()
